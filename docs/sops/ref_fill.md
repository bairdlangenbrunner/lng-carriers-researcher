...
**Last revised:** 2026-09-23 rev 30 (new §3.3b — every vessel checked is also looked up on CSB (yard orderbook, all pages, + the ship page) and agreement/disagreement reported; CSB's "Under Construction" means not yet delivered, and hull digits repeat across yards. Rules A–F unchanged.). Prior: 2026-09-23 rev 29 (§3.8c — a Vessel type value counts only where the article's own text uses it of a vessel: never in a sidebar, navigation, footer or link text, and never before a non-vessel qualifier ("conventional marine fuels"); `corroborates(…, field="Vessel type")` / `vessel_type_statement()`. Rules A–F unchanged.). Prior: 2026-09-22 rev 28 (§5 rewritten — the confidence grade is now computed from the §3.8c gate's own verdict, not from source tier: one ref that passes on a **live** page stating the value for this vessel is **Green**. The 2-source / primary-source ladder is withdrawn. Yellow is now reserved for a gate that passed weakly — archive-only, or a value too generic for a bare text hit — or for a carve-out (§4.18 delivery roll-forward, DF §5a order-total Price, a §3.8c conflict, a researcher `cap_reason`). New `scripts/confidence.py` is the single implementation, wired into `merge_fills.py` and `build_workbook.py --mode fix`; `scripts/regrade_confidence.py` re-grades already-built batches' held lines (promote-only). Rules A–F, §3.8/§3.8a/§3.8c unchanged.). Prior: 2026-09-21 rev 27 (§4.16 — an AIS-transmitted / tracker-reported alternate name also joins `Other names`, gated on the tracker page; per-ref `gate_value` for a cell adding two elements. Rules A–F, §3.8/§3.8c unchanged.). Prior: 2026-09-21 rev 26 (§3.8c — a fleet-table source is gated on the vessel's own row: an IGU report PDF is a ref only for what it prints for the row's IMO in that column, `scripts/igu_refs.py`, wired into every gate caller; a row with no IMO gets no IGU ref; an IGU landing page stands for its edition's PDF — confirm the data point there. Rules A–F unchanged.). Prior: 2026-09-21 rev 25 (new §4.18 — a proposed delivery delay carries a second source: IGU where it prints the same year, else press; shipvault alone stays Yellow. New §4.19 — a later Delivery year carries the former year into `Previous delivery year(s)` and sets `Delivery delayed` = `yes`; `scripts/delivery_history.py`. Rules A–F, §3.8/§3.8c unchanged.). Prior: 2026-09-18 rev 24 (new §4.17 — IGU `(ex-…)` names join `Other names` by IMO; IGU `Name (NNNN)` is the hull; hull numbers always carry a yard tag, `Hull NNNN (Tag)`; `other_names.py --igu-ex`. Rules A–F, §3.8/§3.8c unchanged.). Prior: 2026-09-17 rev 23 (new §4.16 — a proposed Name change carries the former Name into `Other names`: appended with `"; "`, never replacing; hull placeholders count; spelling / truncation corrections, wrong-vessel corrections and placeholder restylings do not; refs gated on the former name and appended; decided together with the Name line. New `scripts/other_names.py` and the fix-mode `append_ref` / `gate_value` cell. Rules A–F and §3.8/§3.8a/§3.8c unchanged.). Prior: 2026-09-17 rev 22 (§4 value-presence pointer — a `Price [ref]` on a per-vessel Price divided out of a reported order total contains the *total*, per Data-fill SOP §5a; the §3.8c gate is run on that total. No other changes.). Prior: 2026-09-17 rev 21 (shipvault companion ref — a shipvault page whose SPA renders blank is cited together with its unit-record URL `https://shipvaultapi-gjb8c.ondigitalocean.app/api/units/{ID}` as a second ref in the same cell (§4.15 join), so a reviewer can see the figures; `url_verifier.py` gates that URL on the same record as the page and renders the record's ISO dates / whole-dollar prices in cell form; new `scripts/shipvault_api_refs.py`. §6a.8 updated. Rules A–F, §3.8/§3.8a/§3.8c unchanged.). Prior: 2026-09-16 rev 20 (Cloudflare pass-through for the vessel-tracker hosts — `fetch.py` now clears the "Attention Required" firewall page via `curl_cffi` TLS impersonation and the JS managed challenge via a real-Chrome `cf_clearance` cookie (`scripts/cf_clearance.py`); `url_verifier.py` gained host adapters that verify shipvault.com and marinetraffic.com SPA pages against the JSON they load; `imo_tracker.py` resolves IMOs through shipvault first, marinetraffic.org second. §6a.8 tracker table + Cloudflare note rewritten (the rev 19 "do not fight the block" text is withdrawn — Baird's 2026-09-16 directive), §9 steps 1/3 updated. Rules A–F, §3.8/§3.8a/§3.8c unchanged.). Prior: 2026-09-16 rev 19 (verifier parity pass — ported the pipelines- / lng-terminals-researcher verifier improvements into `scripts/url_verifier.py` + `scripts/fetch.py`. §3.8 gate now enforces the Forbidden list in code (GEM, abarrelfull, URL shorteners, `web.archive.org/save/`, search / tag / category / author / paginated navigation URLs), re-checks redirect chains (banned host, deep link → site root, id-keyed redirect to a *different* article), verifies PDFs on their extracted text (pdftotext → pypdf → OCR), and matches whitespace-/markup-/diacritic-normalised text with digit-boundary numeric matching. §3.8a rewritten around graded verdicts: **bot-block ≠ dead** — 401/403/429/5xx and Cloudflare/Incapsula/paywall interstitials are `blocked`, get a Wayback-snapshot content check (live URL stays the citation), and are never dropped as dead; `classify()` grades every reason as ok / banned / dead / blocked / uncorroborated and `merge_fills.py` / `build_workbook.py --mode fix` branch on the grade. New `scripts/citation_qc.py` — the §3.8a rot sweep over every existing backend `[ref]` URL (step 0 of a full pass). JSONL audit log via `URL_VERIFIER_LOG` / `--log`. Rules A–F, §3.8b, §3.8c and confidence labels unchanged.). Prior: 2026-06-05 rev 18 (added §3.8c — the value↔ref corroboration gate (hard-block): a `[ref]` may only be cited on a cell whose value its live page actually contains, stricter than §3.8's entity check; a live page stating a *different* figure than the cell carries fails the gate, the ref is dropped, and the conflict is logged for human review — never kept with a soft footnote. Added the dual-figure rule: when a source gives nominal + 98%-fill capacities, the tracker carries the nominal, and a source is only cited for the figure it supports. Correction batches now run through `build_workbook.py --mode fix` + the gate, never hand-edited CSV. Enforced by `corroborates()`/`value_variants()` in `url_verifier.py`, wired into `merge_fills.py` and `build_workbook.py`. Surfaced by the 2026-06-05 rows 1216/1217 capacity defect (176,400 cell citing a 180,000 source). No other research-rule changes; §3.8/§3.8a/§3.8b, Rules A–F, and confidence labels are unchanged.). Prior: 2026-06-03 rev 17 (three output-formatting rules surfaced by the first discovery batch: §4.14 — `Shipowner` / `Operator/charterer` values use the backend's existing short stylization, e.g. write `COSCO`, not `Cosco Shipping Energy Transportation`; §4.15 — multiple URLs in one `[ref]` cell join with `", "`, never a newline; and a §4.8 carve-out — the seven yard-location columns are never *researched* but may be *mirrored* from an existing backend row for the same shipbuilder, per the new Discovery SOP §6.7 autofill. `build_workbook.py` enforces §4.15 and the §6.7 autofill. No research-rule changes; the §3.8 gate, Rules A–F, and confidence labels are unchanged.). Prior: 2026-05-28 rev 16 (repository migration — output model changed from a single rolling xlsx to one committed directory per batch under `batches/`; scratch paths and `present_files` references updated for the Git/Claude Code workflow. No research-rule changes; §3.8 gate, Rules A–F, and confidence labels are all unchanged from rev 15.). Prior: 2026-05-27 rev 15 (added §3.8b — QA notes quote only publicly-visible content; paywalled body text is never quoted in a QA note; editorial entity tags on LNG Prime support yellow confidence as corroboration, not green. Surfaced by a 2026-05-27 F8 finding that quoted paywalled LNG Prime body text where the verifier could only see the public lead and the tag list.).
