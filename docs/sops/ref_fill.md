...
|---|---|---|
| marinetraffic.org | `https://www.marinetraffic.org/ship-owner-manager-ism-data/{YARD-NAME}-{HULL}/{IMO}/1` | Returns title `{YARD-NAME} {HULL} - LNG Tanker, IMO {IMO}`. Cloudflare-blocked from curl since mid-2026 (see caveat below); browser access works. |
...
**Cloudflare caveat (2026-09-16):** all four tracker hosts this step relies on — marinetraffic.org, marinetraffic.com, marinevesseltraffic.com and shipvault.com — now block non-browser clients. marinetraffic.org and marinevesseltraffic.com serve a JS managed challenge ("Just a moment...", `cf-mitigated: challenge`); shipvault.com and marinetraffic.com serve the harder "Attention Required" firewall page. The block is host-side policy, not our IP: local curl with full browser headers and Anthropic-egress WebFetch get the same 403s, and Wayback holds no snapshots of the per-vessel pages. The June 2026 batches that cited these hosts were verified with the same verifier at HTTP 200, so the sites tightened their rules after June. Consequences:
  - **Existing refs stay** — blocked ≠ dead (§3.8a). Confirm a sample in a normal browser (the challenge passes there) and record "environment-blocked — kept"; a fix batch may carry them with `soft: true`.
  - **Do not fight the block in scripts.** No TLS-fingerprint impersonation, proxies or captcha services in `url_verifier.py` / `imo_tracker.py`; a browser check by a human is the only sanctioned route to these hosts.
  - **New citations, 9XXXXXX IMOs:** cite `https://www.vesselfinder.com/vessels/details/{IMO}` instead — curl-friendly, verifier grades it `ok`, title carries name / type / IMO. It corroborates identity cells (Name, IMO, vessel type, DWT) only; builder and hull are blank for newbuilds.
  - **New citations, pre-delivery 1XXXXXX IMOs:** vesselfinder 404s on these (checked 1157109, 1176351); only the marinetraffic-class hosts index them. Cite the marinetraffic.org URL only if a human opened it in a browser this batch (`soft: true`), with a traceability note in the citation log; otherwise fall through to §6a.9.
...
3. If marinetraffic.org gates the request (Cloudflare challenge — the norm since mid-2026), follow the §6a.8 Cloudflare caveat: vesselfinder for 9XXXXXX IMOs, browser-confirmed `soft: true` or §6a.9 for 1XXXXXX IMOs.
