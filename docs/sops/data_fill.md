...
- **Shipowner country/area** ← `data/shipowner_facts.csv` if the owner is listed (non-`AMBIGUOUS`); otherwise the **unambiguous** country shared by sibling backend rows of the same `normalize_owner` (`owner_country()` in `scripts/normalize.py`). If the owner's siblings disagree (currently `mol`: Japan/Türkiye; `maran-gas`: multiple — written `AMBIGUOUS` in the table) or none carry a country, **do not autofill — research it.** The country `[ref]` is copied alongside the value (value + ref travel together → no orphan, Rule F). A country `[ref]` must be a page that **names the country as the owner's base** — a **shipvault.com** page or unit record never is (it prints the owner string and the *vessel's flag*, so it fails §3.8c on every country cell, and where a flag happens to match the country it corroborated the wrong fact); prefer the company's own site, a national registry / exchange filing, an owner-manager database profile, or trade press that states the base. See `data/owner_charterer_map.md`. **A pending batch's copy of a table ref goes stale when the table is re-sourced** — `data_fill.json` holds the URLs as they read at merge time, so before applying a batch whose derivable fills came from a facts table, re-point each fill's `new_urls` at the table's current row for its owner tag, re-run the §3.8c gate, and rebuild the apply artifacts. (2026-09-21: 61 country fills would otherwise have written a dropped shipvault URL back into 36 cells that had just been corrected.)
