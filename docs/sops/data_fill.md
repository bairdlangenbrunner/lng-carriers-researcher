...
**Authoritative facts tables (rev: 2026-06-04).** Builder- and owner-level facts — facts about the *yard* or *owner*, not the vessel — now live in two editable, canonical CSVs that the autofill reads **first**: `refdata/shipbuilder_facts.csv` (yard country/area + the yard-location block, keyed by `normalize_builder`) and `refdata/shipowner_facts.csv` (Shipowner country/area + `[ref]`, keyed by `normalize_owner`). Seed/refresh them from the live backend with `python scripts/seed_lookups.py`; loaders live in `scripts/lookups.py`. The backend-sibling scan below is now the **fallback** for tags absent from a table. `qc_backend.py` flags any backend row whose yard/owner facts disagree with the tables.

- **Shipowner country/area** ← `refdata/shipowner_facts.csv` if the owner is listed (non-`AMBIGUOUS`); otherwise the **unambiguous** country shared by sibling backend rows of the same `normalize_owner` (`owner_country()` in `scripts/normalize.py`). If the owner's siblings disagree (currently `mol`: Japan/Türkiye; `maran-gas`: multiple — written `AMBIGUOUS` in the table) or none carry a country, **do not autofill — research it.** The country `[ref]` is copied alongside the value (value + ref travel together → no orphan, Rule F). See `refdata/owner_charterer_map.md`.
...
- **Yard-location block** (`Shipbuilder yard country/area` + its `[ref]`, and the five `Yard location …` columns) ← `refdata/shipbuilder_facts.csv` first, then the Discovery SOP §6.7 sibling-scan fallback (`_yard_location_map_table_first` → `_build_yard_location_map`, keyed on `normalize_builder`).
