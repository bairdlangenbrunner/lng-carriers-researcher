...
| 4.16 | RF (rev 23) | A proposed Name change also proposes the former Name as an addition to `Other names` (`"; "` join, append-only, refs gated on the former name; not for spelling fixes / wrong-vessel corrections / placeholder restylings) — `other_names.py`, fix-mode `append_ref` cell; rev 27: an AIS-transmitted / tracker-reported alternate name (`Youyi Yongshi` for *Friendship Venture*) also joins, gated on the tracker page |
