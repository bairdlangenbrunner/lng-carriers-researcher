...
**Rows:** 150 live rows, 223 cells (141 / 214 before the marinetraffic.org cross-check, see the last section)
...
52 rows were deliberately NOT proposed (45 from the overnight run, 7 added by the marinetraffic.org cross-check):
...
is pre-filled by confidence: 168 G cells accept, 55 Y cells hold (143 / 71 before the marinetraffic.org cross-check) — flip the holds you agree with
...
95/95 answered 200, no throttling (`work/sep17/marinetraffic_sweep.jsonl`). The other 81 are unnamed
hulls not due before 2028 and were not swept.
