...
| 2026-09-17 | combined | sep-17-pass roll-up of the seven proposal batches + the IGU comparison (review aid, not an apply source) | [folder](2026-09-17_1017ET_sep-17-pass_combined/) | 18 sheets, 1,444 proposals (880 accept / 564 hold) keyed by live sheet row; backend-shape tab; `igu_findings` tab; report-page generator |
...
| 2026-09-17 | fix | Status `scrapped` for the 18 rows IGU 2026 dropped + Puteri Delima Satu → FSU (sep-17-pass 8) | [folder](2026-09-17_1702ET_fix_scrapped_status/) | 19 cells / 19 rows, all G / accept: 18 × Status `active` → `scrapped` (press + shipvault refs; new vocab value, rows are never deleted — IG rev 2), live row 61 Vessel type → `FSU` (two MISC documents) |
