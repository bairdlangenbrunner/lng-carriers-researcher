...
- `<mode>` — `ref_fill`, `discovery`, `data_fill`, `corroborate`, `fix`,
  `qc_<label>`, `sfoc_reconciliation`, `fsru_reconciliation`, `igu_reconciliation`, `combined`
  (a multi-batch roll-up workbook — review aid, not an apply source in its own right).
...
| 2026-09-17 | combined | sep-17-pass roll-up of the eight proposal batches + the IGU comparison (review aid, not an apply source) | [folder](2026-09-17_1017ET_sep-17-pass_combined/) | 20 sheets, 1,912 proposals (1,323 accept / 588 hold / 1 reject) keyed by live sheet row; backend-shape tab; `igu_findings` tab; report-page generator |
...
| 2026-09-17 | fix | what the IGU World LNG Report 2026 prints, the report PDF as sole ref (IG §5.4): blank Vessel / Cargo type, renames, delivery years, propulsion, load corruption (sep-17-pass 8) | [folder](2026-09-17_1654ET_fix_igu2026_sourced/) | 468 cells / 328 rows (443 accept, 24 hold, 1 reject); 32 manual-review values (vocabulary / stylization picks); apply via `apply_patch.csv` |
| 2026-09-17 | fix | Status `scrapped` for the 18 rows IGU 2026 dropped + Puteri Delima Satu → FSU (sep-17-pass 9) | [folder](2026-09-17_1702ET_fix_scrapped_status/) | 19 cells / 19 rows, all G / accept: 18 × Status `active` → `scrapped` (press + shipvault refs; new vocab value, rows are never deleted — IG rev 2), live row 61 Vessel type → `FSU` (two MISC documents) |
| 2026-09-17 | fix | former Names → `Other names`, retroactive on batches 1/2/8 (RF §4.16, sep-17-pass 10) | [folder](2026-09-17_1737ET_fix_other_names_former/) | 160 cells / 160 rows: 124 G accept, 1 Y accept, 35 Y hold; rebuilt 2026-09-18 with IGU `(ex-…)` names + builder-tagged hulls |
| 2026-09-17 | fix | Vessel type `qc-max` for the 24 IGU-typed QatarEnergy 271,000 cbm ships (sep-17-pass 11) | [folder](2026-09-17_1809ET_fix_qcmax_vessel_type/) | 24 cells / 24 rows, all G / accept; new vocab value `qc-max`, IGU 2026 PDF sole ref |
| 2026-09-17 | fix | Price in full US dollars, `$m` → `USD` (Baird ruling 2026-09-17, sep-17-pass 12) | [folder](2026-09-17_1810ET_fix_price_full_usd/) | 58 cells / 29 rows, all G / accept, `preserve_ref` unit conversion; `$m` retired from the vocab |
| 2026-09-18 | fix | Hull numbers from IGU 2026 `Name (hull)` entries (RF §4.17, sep-17-pass 13) | [folder](2026-09-18_2005ET_fix_igu2026_hulls/) | 41 cells / 41 rows, all G / accept: 13 blank Hull number fills + 28 untagged hulls restyled with a yard tag |
| 2026-09-21 | fix | review-app suggestion, hand-built (sep-17-pass 14) | [folder](2026-09-21_1740ET_fix_review_suggestions/) | 1 cell, G accept: tracker-reported alternate name added to `Other names` beside the former Name |
| 2026-09-21 | fix | `Shipowner country/area [ref]` off shipvault, replaced (shipvault states owner + flag, never country) | [folder](2026-09-21_2003ET_fix_shipowner_country_refs/) | 55 rows: 53 accept (ref swap only), 2 hold (Hanwha Ocean vs Hanwha Shipping LLC conflict); applied 2026-09-21, holds later closed by Baird's own edit |
| 2026-09-22 | fix | Hull 2663 (SHI) named Christy Vanguard (reviewer catch) | [folder](2026-09-22_1912ET_fix_christy_vanguard/) | 1 row, 2 cells (Name Y, Other names Y); single shipspotting-photo source corroborated by 2 IGU PDFs on the former name |
| 2026-09-22 | fix | shipvault hull numbers and delivery years, whole-backend shipvault sweep | [folder](2026-09-22_2208ET_fix_shipvault_hulls/) | 212 shipvault-cited rows re-fetched and compared field by field; 5 real defects (3 hull-number corrections, 2 delivery-year rolls forward with §4.19 companions) |
| 2026-09-23 | fix | unsourced Vessel type `conventional` (Baird: every proposed value needs a ref that states it) | [folder](2026-09-23_1230ET_fix_vessel_type_unsourced/) | 16 rows: 7 ref-swapped to the IGU 2026 PDF (value unchanged), 9 unsourced `conventional` cleared; applied via directed session write (AP §2d), verified 0 mismatch |
| 2026-09-23 | discovery | comprehensive whole-orderbook + whole-fleet reconciliation, 7 main + 14 secondary yards, expanded proposed bucket (Discovery SOP §2) | [folder](2026-09-23_1546ET_discovery_comprehensive/) | 1 candidate (Maran Gas Efessos, IMO 9627497, seeding gap); 31 backend status flags; 0 new CSB rows; dedupe 0 HIGH |
| 2026-09-23 | fix | Mozambique builder/owner + BW three-tank capacity (from the S4/S5 discovery re-run) | [folder](2026-09-23_1707ET_fix_mozambique_bw/) | 19 rows / 28 cells, 0 refs dropped: 9 Shipbuilder G (HHI → HD Hyundai Samho), 17 Shipowner Y-capped (`MOL, NYK` → `MOL, K Line` / `NYK Line, Maran Gas Maritime`), 2 Capacity G (174000 → 177000); ships a `url_verifier.py` multi-owner gate fix |
