"""
Push accepted lines into the backend sheet — the one place the review app writes the backend.

Never on a click of `accept`: the reviewer presses **push accepted**, the server pulls the
...
and a data-fill / ref-fill value whose cell is no longer blank or `unknown` (additive to blanks
...
def plan(data, dirs, backend_path=None, batch=None):
    ...
    for (rid, column), (key, b) in source.items():
        ...
        w = {"row_id": rid, "live_row": srm[rid], "column": column, "a1": a1(i, srm[rid]),
             "old": old, "new": new, "key": key, "batch": b["dir"], "label": b["label"],
